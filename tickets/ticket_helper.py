...
def create_ticket(ticket):
    ...
    if isinstance(ticket, HardwareTicket):
        query = """
            INSERT INTO t_HardwareTicket (id, equipamento, avaria, descRep, pecas, ticketID)
            VALUES (%s, %s, %s, %s, %s, %s)
            """
        ...
        query = """
            INSERT INTO t_SoftwareTicket (id, software, descNecessidade, descInt, ticketID)
            VALUES (%s, %s, %s, %s)
            """
        cursor.execute(query, (
    ...
    query = """
        INSERT INTO t_Tickets (id, dtCriacao, idColaborador, estTicket, estAtendimento, Tipo)
        VALUES (%s, %s, %s, %s, %s, %s)
        """
    cursor.execute(query, (
        str(ticket.id), ticket.dtCriacao, ticket.idColaborador, ticket.estTicket, ticket.estAtendimento, ticket.Tipo))
